    ...
    for inst in instructions {
        match &inst {
            ...
            AsmInstruction::PushLabel(_) => {
                // PushLabel must be preserved outside egglog optimization.
                // Flush the current block, emit PushLabel as its own block body.
                blocks.push(BasicBlock {
                    label: current_label.take(),
                    comments: std::mem::take(&mut current_comments),
                    body: std::mem::take(&mut current_body),
            ...
            AsmInstruction::Op(op) if *op == Opcode::Jump => {
                // Raw JUMP (e.g., function return) terminates the block
                blocks.push(BasicBlock {
                    label: current_label.take(),
                    comments: std::mem::take(&mut current_comments),
                    body: std::mem::take(&mut current_body),
